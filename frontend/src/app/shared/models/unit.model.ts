  ...
  calculateAverageRatings(): void {
    if (this.reviews.length > 0) {
      this.avgOverallRating =
        this.reviews.reduce((sum, review) => sum + review.overallRating, 0) /
        this.reviews.length;
      this.avgRelevancyRating =
        this.reviews.reduce((sum, review) => sum + review.relevancyRating, 0) /
        this.reviews.length;
      this.avgFacultyRating =
        this.reviews.reduce((sum, review) => sum + review.facultyRating, 0) /
        this.reviews.length;
      this.avgContentRating =
        this.reviews.reduce((sum, review) => sum + review.contentRating, 0) /
        this.reviews.length;
    } else {
      this.ensureDefaults();
    }
