...
export class ProfileComponent implements OnInit, OnDestroy {
  ...
  @Output() stateChangeEvent = new EventEmitter<
    'logged in' | 'signed up' | 'logged out' | 'signed out' | 'forgot password'
  >();
  @Output() createToast = new EventEmitter<{
    severity: string;
    summary: string;
    detail: string;
  }>();
  ...
  ngOnInit(): void {
    ...
    this.authService.validateSession().subscribe({
      next: (response) => {
        ...
        this.createToast.emit({
          severity: 'success',
          summary: 'Validated Session & Logged in',
      ...
      error: (error) => {
        ...
        this.createToast.emit({
          severity: 'warn',
          summary: 'Session Expired & Logged out',
    ...
    if (this.state == 'signed up') {
      ...
      this.titleChangeEvent.emit('Profile');
    } else if (this.state == 'signed out') {
      this.titleChangeEvent.emit('Sign Up');
    } else if (this.state == 'logged out') {
      this.titleChangeEvent.emit('Login');
    }
  }
      ...
      error: (error: HttpErrorResponse) => {
        if (error.status == 409) {
          this.createToast.emit({
            ...
            detail:
              'Account exists as non-Google user, please sign in manually.',
          });
        } else if (error.status == 403) {
          this.createToast.emit({
            severity: 'warn',
            summary: 'Invalid email',
            detail: 'Please sign in using a Monash University email.',
          });
        }
  ...
  updateDetails() {
    ...
    if (usernameRegex.test(updatePayload.username || this.user.username)) {
        ...
        this.authService
          ...
          .subscribe({
            next: (response) => {
              ...
              const userItem = this.profileMenuItems.find((item) =>
                item.label?.startsWith('User:')
              );
              if (userItem) {
      ...
      this.createToast.emit({
        ...
        detail:
          'Invalid Username (Please enter an alphanumeric username <= 20 characters)',
      });
        ...
        this.authService.uploadAvatar(file, this.user.email).subscribe({
          ...
          error: (error) => {
            ...
            this.createToast.emit({
              ...
              detail: error.message || 'Failed to upload avatar',
            });
          },
        });
      }
    ...
    this.authService.deleteUserAccount(this.user._id.toString()).subscribe({
      ...
      error: (error) => {
        this.createToast.emit({
          ...
          detail: 'There was an error whilst deleting your account',
        });
  ...
  getUserReviews(userId: any) {
    this.apiService.getUserReviewsGET(userId).subscribe({
      next: (reviews: Review[]) => {
        ...
        if (this.user) this.user.reviews = this.reviews;
    ...
    this.apiService.deleteReviewByIdDELETE(reviewId).subscribe({
      ...
      error: (error: any) => {
        ...
        this.createToast.emit({
          ...
          detail: 'There was an error whilst deleting your review',
        });
  ...
  navigateMenu(direction: 'prev' | 'next') {
    ...
    if (direction === 'next') {
      ...
      newIndex =
        (currentIndex - 1 + this.menuStates.length) % this.menuStates.length;
    }

    this.profileMenuState = this.menuStates[newIndex] as
      | 'details'
      | 'reviews'
      | 'friends'
      | 'settings';
